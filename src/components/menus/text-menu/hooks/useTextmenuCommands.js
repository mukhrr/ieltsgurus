...
export const useTextmenuCommands = (editor) => {
  ...
  const onChangeColor = useCallback((color) => editor.chain().setColor(color).run(), [editor])
  const onClearColor = useCallback(() => editor.chain().focus().unsetColor().run(), [editor])

  const onChangeHighlight = useCallback((color) => editor.chain().setHighlight({ color }).run(), [editor])
